...
document.getElementById('searchInput').addEventListener('keydown', (event) => {
  if (event.key === 'Enter') {
    ...
    const target = query.includes('globoplay') || query.includes('amazon') || query.includes('deezer') || query.includes('disney') || query.includes('hbo') || query.includes('sky+') || query.includes('aplicativo') ? 'guias' : query.includes('sva') || query.includes('multa') || query.includes('regra') || query.includes('cancel') || query.includes('formaliza') || query.includes('financeiro') ? 'regras' : query.includes('compos') || query.includes('equipamento') && query.includes('valor') ? 'composicao' : query.includes('plano') || query.includes('atualiza') ? 'atualizacoes' : query.includes('endereço') || query.includes('titular') || query.includes('cabo') || query.includes('equipamento') ? 'processos' : query.includes('mensagem') || query.includes('fala') ? 'colinhas' : query.includes('vídeo') ? 'videos' : 'atendimentos';
    showView(target);
  }
});
document.addEventListener('keydown', (event) => { if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === 'k') { event.preventDefault(); document.getElementById('searchInput').focus(); } });
